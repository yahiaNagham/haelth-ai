...
export default function DevicesTable() {
  ...
  {name: "USER ID", uid: "user", sortable: true},
...
//   const deleteDeviceFormTable = React.useCallback((id:string) => {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredDevices = filteredDevices.filter((device) =>
        device.imei.toLowerCase().includes(filterValue.toLowerCase()),
      );
    }
  ...
  const renderCell = React.useCallback((device: Device, columnKey: React.Key) => {
    ...
    switch (columnKey) {
      ...
      case "user":
        return (
          <div className="flex flex-col">
            <p className="text-bold text-small capitalize">{device.user_id}</p>
          </div>
        );
      case "actions":
        return (
          <div className="relative flex justify-start items-center gap-2">
            ...
            <DeleteModal deviceId={device.id} deleteDevice={deleteDeviceFormTable}/> */}
          </div>
        );
      default:
        return cellValue;
